...
function searchData() {
    ...
    // Jika input kosong → matikan loader & skeleton
    if (!raw) {
        hideSkeleton();
        hideLoader();
        return renderError("Masukkan nama atau angka.");
    }
    ...
    fetch(`${API_URL}?q=${encodeURIComponent(q)}`)
        ...
        .catch(() => {
            hideSkeleton();
            hideLoader();
            renderError("Server tidak merespon.");
        });
}
...
<div class="result-item">
    <div><strong>Nama:</strong> 
        <span class="badge badge-name">${escape(r.nama)}</span>
    </div>

    <div><strong>Nomor WhatsApp:</strong> 
        <span class="badge badge-wa">${escape(r.nomor_whatsapp)}</span>
    </div>

    <div class="small"><strong>Nomor Lapak:</strong> 
        <span class="badge badge-lapak">${escape(r.nomor_lapak)}</span>
    </div>
</div>`;
